...
class ResNet(nn.Module):
    def __init__(self, option='resnet18', pret=True, num_classes=12,num_layer = 2,prob=0.5,middle=1000):
        super(ResNet, self).__init__()
        self.res_base = ResBase(option, pret)
        self.res_classifier = ResClassifier(num_classes, num_layer, self.res_base.dim, prob, middle)

    def forward(self, input, reverse=False):
        x = self.res_base(input)
        x = self.res_classifier(x, reverse)
